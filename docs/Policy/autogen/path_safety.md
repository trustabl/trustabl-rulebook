...
# Policy Rationale: AutoGen Filesystem Path Safety

**Policy ID:** `autogen_path_safety`  
**File:** `autogen/path_safety.yaml`  
**Rules:** AG2-014  
**Severities:** high  
**Fix types:** code  
...
> [mcp/path_safety.md](../mcp/path_safety.md). AutoGen-specific angle only.

---

## What this policy covers

A AutoGen tool — a function registered with `register_function` / `register_for_llm` — that takes a path-like parameter and passes it to a
...
AutoGen fills the argument from the shared conversation, and in a `GroupChat`
every participant writes into it. A path proposed by any agent — including one
whose job is summarizing untrusted content, and including one an operator added
without reviewing the file tools another agent holds — reaches this function.
The conversation is also durable, so a traversal string introduced once stays in
history and can be re-selected on a later turn.
...
### AG2-014 — AutoGen tool uses a caller-supplied path without containment (Severity: high, Confidence: 0.7, Fix type: code)
