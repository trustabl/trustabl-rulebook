---
policy_id: pydantic_ai_path_safety
...
# Policy Rationale: Pydantic AI Filesystem Path Safety

**Policy ID:** `pydantic_ai_path_safety`  
**File:** `pydantic_ai/path_safety.yaml`  
**Rules:** PYD-009  
**Severities:** high  
**Fix types:** code  
...
> [mcp/path_safety.md](../mcp/path_safety.md). Pydantic AI-specific angle only.

---

## What this policy covers

A Pydantic AI tool — a function registered with `@agent.tool` / `@agent.tool_plain`, or passed via `tools=` — that takes a path-like parameter and passes it to a
...
Pydantic AI's whole premise is that arguments are validated, and they are — against
their annotation. That is exactly what makes this the framework where the gap is
easiest to miss. `str` accepts `../../etc/passwd`. So does `Path`. The annotation
constrains what the value *looks like*, never where it *points*, and a tool author
who has seen the framework reject a malformed argument reasonably but wrongly
concludes that a well-formed one is safe. The model fills the argument from its
context, so the value is attacker-reachable wherever that context is.
...
### PYD-009 — Pydantic AI tool uses a caller-supplied path without containment (Severity: high, Confidence: 0.7, Fix type: code)
...
5. A Pydantic field validator is a good home for the check, provided it validates the
*resolved* path — a validator on the raw string still lets symlinks and `..`
through, and inherits the same false sense of safety.
