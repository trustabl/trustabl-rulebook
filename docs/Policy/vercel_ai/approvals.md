...
calls — and the fact that its default is `false` everywhere. **VAI-013** (tool
scope) fires when a `tool()` or `dynamicTool()` whose `execute()` body shells
out, evaluates code, or writes the filesystem has no effective `needsApproval`
gate — the option is absent or explicitly `false`. Passing `true` or a per-call
approval function both count as a gate and do not fire. This complements
capability rules **VAI-001** (subprocess), **VAI-002** (eval / `new Function`),
...
A privileged Vercel AI tool — one whose `execute()` runs a command, evaluates
code, or mutates files — is exactly the kind of action that warrants human
confirmation before it fires, because its effects are real and hard to undo. The
...
unless the author opts in — the safe behavior is the one you have to remember to
ask for.
...
shell, an in-process evaluator, or the filesystem with no checkpoint. This is
...
**What we detect:** a `vercel_ai_tool` whose `execute()` body shells out, runs
`eval` / `new Function`, or writes the filesystem (`has_shell_call` /
`has_code_exec_call` / `has_write_call`) and has no effective `needsApproval`
gate — the option is absent or explicitly `false`; `true` or a callback
function silences the rule.
...
`rm -rf` chain. The same applies to an `eval(userExpr)` helper or a write tool
that overwrites config files: the missing gate is the difference between
"model proposed" and "system executed."
...
amplifiers and hygiene; an absent approval gate on shell/code/write is a direct
...
await streamText({
  model: yourModel,
  tools: { deploy },
  // SDK 7+: toolApproval: { deploy: "user-approval" },
});
```
...
4. Credit threat-model review: Karlee (hackathon coordination).
