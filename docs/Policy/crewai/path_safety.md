...
# Policy Rationale: CrewAI Filesystem Path Safety

**Policy ID:** `crewai_path_safety`  
**File:** `crewai/path_safety.yaml`  
**Rules:** CREW-008  
**Severities:** high  
**Fix types:** code  
...
> [mcp/path_safety.md](../mcp/path_safety.md). CrewAI-specific angle only.

---

## What this policy covers

A CrewAI tool — a `@tool`-decorated function or a `BaseTool` `_run` body — that takes a path-like parameter and passes it to a
...
A CrewAI agent chooses the argument from its context, and in a crew that context
includes other agents' task output. A researcher agent that summarizes a fetched
page, and hands its summary to an agent holding a file tool, is a complete path
from untrusted content to `open()` — with the added property that the agent
making the file call never saw the original source and has no basis for judging
it. Delegation widens the reachable set with every hop.
...
### CREW-008 — CrewAI tool uses a caller-supplied path without containment (Severity: high, Confidence: 0.7, Fix type: code)
